...
var deathCount = 0  // Ölüm sayacı — 5. ölümde reklam gösterecek
...
func gameOver() {
    ...
    // MARK: - Ölüm sayacını artır
    deathCount += 1
    print("💀 Ölüm sayısı: \(deathCount)")
...
func addResultsAndButtons() {
    if canShowScore{
        ...
        // MARK: - 5. ölümde reklam göster
        if deathCount >= 5 {
            print("🎬 5. ölüm! Reklam gösteriliyor...")
            showAdViewController()
        }
    }
}

// MARK: - Reklam ViewContoller'ı göster
private func showAdViewController() {
    guard let window = UIApplication.shared.windows.first else { return }
    guard let rootViewController = window.rootViewController else { return }
    
    let adViewController = AdViewController()
    adViewController.modalPresentationStyle = .fullScreen
    adViewController.modalTransitionStyle = .crossDissolve
    
    rootViewController.present(adViewController, animated: true) {
        print("✅ Ad controller sunuldu")
    }
}
